try {
	$deviceinfo = get-nableDevice
} catch {
	Write-Host "Failed to get device info from N-Able: " $_.Exception.Message
}
...
try {
	...
	if ($env:userdomain -ne "WORKGROUP") {
		$params = "/s /v`" /qn CUSTOMERID=$custid CUSTOMERNAME=$custname SERVERPROTOCOL=HTTPS SERVERADDRESS=n-able.secureworkplace.net SERVERPORT=443 PROBETYPE=Network_Windows AGENTDOMAIN=$env:userdomain AGENTUSERNAME=$agentusername AGENTPASSWORD=$agentpassword `""
	} else {
		$params = "/s /v`" /qn CUSTOMERID=$custid CUSTOMERNAME=$custname SERVERPROTOCOL=HTTPS SERVERADDRESS=n-able.secureworkplace.net SERVERPORT=443 PROBETYPE=Workgroup_Windows AGENTUSERNAME=$agentusername AGENTPASSWORD=$agentpassword `""
	}
